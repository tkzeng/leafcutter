...
make_cluster_plot <- function(
  ...
  for( fancyVar in 1:length(groups) ){
    ...
    allEdges=do.call(rbind,foreach (i=1:nrow(intron_meta)) %do% {
      ...
      # label each junction
      edge$label=paste0(format(intron_meta$prop[i],digits=2, scientific=FALSE))
...
for (i in 2:length(plots))
    plots[[i]] <- plots[[i]] + scale_colour_manual("", values = mainPalette ) + theme(legend.position="bottom", legend.justification = 'right')
